...
- [x] Cleanup polish: add 300 ms sync barrier in `group_remove_all` before cleanup (device state race), widen brute-force range to 1-63 (sparse IDs on real devices), elevate tier logs to INFO for runtime visibility.
...
- [x] Fix root cause of permanent "keyset table full": `group_remove` and `group_remove_all` never wrote back an updated GroupKeyMap after removing groups. Cleanup read GroupKeyMap, saw the old bindings as "still referenced", and removed nothing. Fix: both methods now (1) read `Descriptor.PartsList`, (2) send `RemoveGroup` to ALL endpoints, (3) strip the affected group_ids from GroupKeyMap and write it back, then (4) run keyset cleanup. Added `_get_node_endpoints` helper. This ensures consistent state: `group_list` on any endpoint reflects reality and keyset slots are always reclaimed.
...
- [x] Fix `group_reset_node` and Tier-3 cleanup missing server-assigned keyset IDs: `keyset_id = (group_id % 0xFFFE) + 1` produces IDs >> 63 (e.g. group_id=100 → keyset_id=101). The brute-force range(1, 64) missed them entirely, so "removed 0 keysets" every time. Fix: `group_reset_node` now collects targeted IDs from tracker + key_store BEFORE clearing them, removes those explicitly, then brute-forces range(1,64) for any others. Also writes empty GroupKeyMap to device. Tier-3 in `_cleanup_unused_keysets_on_node` now adds `_group_key_store` keyset IDs to the brute-force set.
