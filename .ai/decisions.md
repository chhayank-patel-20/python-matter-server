...
### 2. Pre-send `KeySetWrite` in `send_group_command` — always, not conditional
- **Decision**: Before sending the multicast, `send_group_command` calls `_ensure_keyset_on_node` for every node in `_group_provisioned_nodes[group_id]`. `_ensure_keyset_on_node` calls `KeySetWrite` unconditionally — it does NOT check `_known_keysets_per_node` first. Failures are caught by `_ensure_keyset_on_node` and logged as warnings so a single unreachable node does not block the multicast for all others.
- **Rationale**: `_known_keysets_per_node` is a controller-side assumption. It does not reflect device truth after a device reboot, firmware update, or factory reset — the tracker still shows the keyset as present while the device has lost it. Relying on the tracker to decide whether to skip `KeySetWrite` is therefore operationally fragile and produces silent drops. `KeySetWrite` is idempotent per Matter spec §11.2.7.1.1: if the keyset already exists, the device overwrites it in-place — so calling it unconditionally is safe and always produces the correct device state. This removes the need to infer device state at all.

### 2a. `_ensure_keyset_on_node` helper
- **Decision**: Extracted the unconditional `KeySetWrite` call into a private `_ensure_keyset_on_node(node_id, keyset_id, epoch_key_hex)` coroutine. Catches all exceptions internally and logs them as warnings. Calls `group_add_key_set` (which also updates `_known_keysets_per_node` tracker on success).
- **Rationale**: Keeps `send_group_command` readable and separates the "enforce device state" concern from the multicast dispatch logic. The tracker is still updated on success so cleanup code continues to work correctly.
